(() => {
  ...
  (() => {
    ...
    const t = new Set(),
      ...
      a = e => {
        return e || ((t = r.runtime), [r.prefix, t, r.suffix].filter(e => e && e.length > 0).join("-"));
        var t;
      };
    function n(e, t) {
      const s = new URL(e);
    ...
    s(3931);
    const o = e => (e && "object" == typeof e ? e : { handle: e });
    class c {
    ...
    class l {
      ...
      handleRequest({ request: e, event: t }) {
        ...
        const r = s.origin === location.origin,
          { params: a, route: n } = this.findMatchingRoute({ event: t, request: e, sameOrigin: r, url: s });
        let i = n && n.handler;
        const o = e.method;
      ...
      findMatchingRoute({ url: e, sameOrigin: t, request: s, event: r }) {
        ...
        for (const n of a) {
          let a;
          const i = n.match({ url: e, sameOrigin: t, request: s, event: r });
          if (i)
            return (
              (a = i),
      ...
      unregisterRoute(t) {
        if (!this._routes.has(t.method))
          throw new e("unregister-route-but-not-found-with-method", { method: t.method });
    ...
    class d {
      ...
      async fetch(t) {
        ...
        try {
          ...
          for (const t of this.iterateCallbacks("fetchDidSucceed")) e = await t({ event: s, request: n, response: e });
          return e;
        } catch (e) {
          throw (
            (a &&
      ...
      async cacheMatch(e) {
        ...
        const { cacheName: r, matchOptions: a } = this._strategy,
          ...
          i = Object.assign(Object.assign({}, a), { cacheName: r });
        s = await caches.match(n, i);
        for (const e of this.iterateCallbacks("cachedResponseWillBeUsed"))
          s = (await e({ cacheName: r, matchOptions: a, cachedResponse: s, request: n, event: this.event })) || void 0;
        return s;
      }
      async cachePut(s, r) {
        const a = f(s);
        ...
        const { cacheName: h, matchOptions: l } = this._strategy,
          ...
          p = d
            ? await (async function (e, t, s, r) {
                ...
                const i = Object.assign(Object.assign({}, r), { ignoreSearch: !0 }),
                  o = await e.keys(t, i);
                for (const t of o) if (a === n(t.url, s)) return e.match(t, r);
        ...
        for (const e of this.iterateCallbacks("cacheDidUpdate"))
          await e({ cacheName: h, oldResponse: p, newResponse: c.clone(), request: i, event: this.event });
        return !0;
      }
      async getCacheKey(e, t) {
        ...
        if (!this._cacheKeys[s]) {
          ...
          for (const e of this.iterateCallbacks("cacheKeyWillBeUsed"))
            r = f(await e({ mode: t, request: r, event: this.event, params: this.params }));
          this._cacheKeys[s] = r;
        }
        ...
        for (const t of this._strategy.plugins)
          if ("function" == typeof t[e]) {
            const s = this._pluginStateMap.get(t),
              r = r => {
                const a = Object.assign(Object.assign({}, r), { state: s });
                return t[e](a);
              };
            yield r;
          }
    ...
    class p {
      ...
      handleAll(e) {
        ...
        const t = e.event,
          ...
          a = new d(this, { event: t, request: s, params: r }),
          n = this._getResponse(a, s, t);
        return [n, this._awaitComplete(n, a, s, t)];
      }
      async _getResponse(t, s, r) {
        let a;
        await t.runCallbacks("handlerWillStart", { event: r, request: s });
        try {
          if (((a = await this._handle(s, t)), !a || "error" === a.type)) throw new e("no-response", { url: s.url });
        } catch (e) {
          if (e instanceof Error)
        ...
        for (const e of t.iterateCallbacks("handlerWillRespond")) a = await e({ event: r, request: s, response: a });
        return a;
      }
      async _awaitComplete(e, t, s, r) {
        ...
        try {
          await t.runCallbacks("handlerDidRespond", { event: r, request: s, response: a }), await t.doneWaiting();
        } catch (e) {
          e instanceof Error && (n = e);
        }
        if (
          (await t.runCallbacks("handlerDidComplete", { event: r, request: s, response: a, error: n }), t.destroy(), n)
        )
          throw n;
      }
    }
    self.addEventListener("activate", () => self.clients.claim());
    const g = ["image", "video", "audio", "script", "style", "font"],
      w = ["/"];
    !(function (e, t, s) {
      let r;
      (r = e instanceof RegExp ? new h(e, t, s) : new c(e, t, s)),
        (u || ((u = new l()), u.addFetchListener(), u.addCacheListener()), u).registerRoute(r);
    })(
      e => {
        let { request: t, url: s } = e;
        return s.origin === self.location.origin && (g.includes(t.destination) || w.includes(s.pathname));
      },
      new (class extends p {
        async _handle(t, s) {
          let r,
          ...
          if (!a) throw new e("no-response", { url: t.url, error: r });
          return a;
        }
